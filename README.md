...
| 공급업체 | 지역 | 특징 | API 지원 |
|---------|------|------|---------|
| **LCSC** | 중국/글로벌 | JLCPCB 부품 공급처, 가격 경쟁력 | 공식 Partner API (심사, 이용약관 제약) |
| **JLCPCB Parts** | 중국/글로벌 | PCB Assembly 전용 라이브러리, Basic/Extended 구분 | 공식 API (심사 기반) |
...
> **Phase 0 완료 → Phase 1 준비 중 (DB 파이프라인 구축 완료)**

조사/분석/검증을 마치고, 첫 번째 데이터 소스(CDFER/jlcpcb-parts-database SQLite)를 확정했습니다. DB 분석과 성능 최적화 실험을 완료하고, 재현 가능한 DB 준비 스크립트를 구축했습니다.

### 진행 문서

| 문서 | 내용 |
|------|------|
| [`연구조사_전자부품_검색사이트.md`](./연구조사_전자부품_검색사이트.md) | 6개 사이트 조사, GitHub 프로젝트 분석 |
| [`검증결과_데이터소스비교.md`](./검증결과_데이터소스비교.md) | API 검증, 데이터 소스 옵션 비교 |
| [`STEP2_DB분석_보고서.md`](./STEP2_DB분석_보고서.md) | CDFER SQLite 실제 구조/데이터 분석 |
| [`STEP3_DB최적화_보고서.md`](./STEP3_DB최적화_보고서.md) | FTS rebuild, 인덱스, 가격 정렬 실험 |
| [`STEP4_DB준비스크립트_보고서.md`](./STEP4_DB준비스크립트_보고서.md) | 자동화 스크립트 결과 |

### 첫 번째 데이터 소스

**CDFER/jlcpcb-parts-database** (SQLite, ~2GB, 633,341 부품)

- 원본: yaqwsx/jlcparts에서 stock ≥ 5인 부품만 추출
- 92개 카테고리, 706개 서브카테고리, 1,417개 제조사
- 수량별 가격 래더, 데이터시트 URL, 기술 사양 (attributes JSON) 포함
...
- **공식 Partner API 존재** (https://www.lcsc.com/docs/)
  - 파트너 대상 8개 API 서비스 (Keyword Search, Category, Item Details 등)
  - 신청 조건: 사업자등록증 + 회사 웹사이트 + 예상 주문량 등 제출
  - 인증: SHA1 서명 + IP 화이트리스트
  - Rate Limit: 200/min, 1000/day
  - **이용약관 제약**: 대량 데이터 수집 금지, 공개 서비스 통합 금지, 데이터 재배포 금지
- **구형 비공식 API (폐쇄됨)**: `wwwapi.lcsc.com` — DNS 해석 실패, 완전 폐쇄 확인

#### JLCPCB
- **공식 Components API**: https://api.jlcpcb.com/
- HMAC-SHA256 인증 (심사 기반 발급 — 주문 이력/비즈니스 관계 기준, 개인 프로젝트 승인 어려움)
- 실시간 가격, 재고, 사양, Basic/Extended 구분 제공
- **오픈소스 데이터**: yaqwsx/jlcparts 프로젝트가 부품 DB를 SQLite로 하루 3회 업데이트 (~11GB)

#### jlcsearch (tscircuit/jlcsearch)
- **무료 공개 API** (인증 불필요): https://jlcsearch.tscircuit.com/
- jlcparts 데이터 기반, 매일 업데이트
- 45개 카테고리 파라미터 검색 + 전체 키워드 검색 (search_index, stock > 0)
- 데이터 범위: jlcparts가 현재 인덱싱한 in-stock 부품 (전체 JLCPCB 라이브러리 coverage 보장 아님)
...
- [x] 활용 가능한 리소스 정리
- [x] API 동작 실제 검증 (LCSC 비공식 폐쇄 확인, jlcsearch 정상 확인)
- [x] 데이터 소스 옵션 비교 분석 및 이용약관 호환성 확인
- [x] 첫 번째 데이터 소스 확정 (CDFER SQLite)
- [x] DB 구조 및 데이터 품질 분석 (STEP 2)
- [x] DB 성능 최적화 실험 (STEP 3: FTS, 인덱스, 가격 정렬)
- [x] 재현 가능한 DB 준비 스크립트 구축 (STEP 4)
- [ ] 통합 데이터 모델 설계
- [ ] 기술 스택 결정
- [ ] 시스템 아키텍처 설계
...
새 CDFER DB를 받았을 때 최적화를 적용하는 스크립트:

```
python scripts/prepare_db.py <원본DB> <출력DB>
```

적용 내용: FTS5 rebuild, 인덱스 7개, first_price 컬럼. 자동 검증 포함 (~95초).

자세한 내용: [`STEP4_DB준비스크립트_보고서.md`](./STEP4_DB준비스크립트_보고서.md)
...
├── README.md                          # 이 파일
├── scripts/
│   └── prepare_db.py                  # DB 준비 스크립트 (최적화 자동 적용)
├── 연구조사_전자부품_검색사이트.md          # 상세 조사 보고서
├── 검증결과_데이터소스비교.md             # API 검증 결과 및 데이터 소스 비교 분석
├── STEP2_DB분석_보고서.md              # CDFER SQLite 구조/데이터 분석
├── STEP3_DB최적화_보고서.md             # 성능 최적화 실험 결과
├── STEP4_DB준비스크립트_보고서.md         # DB 준비 자동화 결과
├── .gitignore
└── (추후 구현 코드)
```
